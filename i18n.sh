...
cloud_msg_russian() {
  case "$1" in
    ...
    config_title) printf '%s\n' 'Настройка Cloud Zimlet 3.0.2 (Nextcloud + офисный сервер)' ;;
    ...
    install_done) printf '%s\n' 'Установка 3.0.2 завершена. mailboxd работает, расширение сообщает ожидаемую версию.' ;;
...
cloud_msg_extended() {
  ...
  case "$1" in
    ...
    config_title) values='Configuração da Zimlet Cloud 3.0.2 (Nextcloud + servidor de escritório)|Configuração da Zimlet Cloud 3.0.2 (Nextcloud + servidor de escritório)|Cloud Zimlet 3.0.2 कॉन्फ़िगरेशन (Nextcloud + ऑफ़िस सर्वर)|Konfigurasi Zimlet Cloud 3.0.2 (Nextcloud + pelayan pejabat)' ;;
    ...
    install_done) values='Instalação 3.0.2 concluída. O mailboxd está operacional e a extensão indica a versão esperada.|Instalação 3.0.2 concluída. O mailboxd está em execução e a extensão informa a versão esperada.|इंस्टॉलेशन 3.0.2 पूर्ण। mailboxd चल रहा है और एक्सटेंशन अपेक्षित संस्करण बता रहा है।|Pemasangan 3.0.2 selesai. mailboxd sedang berjalan dan sambungan melaporkan versi yang dijangka.' ;;
    ...
    clean_modern) values='Implementação limpa da interface Modern 3.0.2…|Implantação limpa da interface Modern 3.0.2…|Modern 3.0.2 इंटरफ़ेस की साफ़ तैनाती…|Penggunaan bersih antara muka Modern 3.0.2…' ;;
    ...
    ui_repaired) values='Interface 3.0.2 implementada sem alterar a extensão Java, a configuração ou os perfis.|Interface 3.0.2 implantada sem alterar a extensão Java, a configuração ou os perfis.|इंटरफ़ेस 3.0.2 Java एक्सटेंशन, कॉन्फ़िगरेशन या प्रोफ़ाइल बदले बिना तैनात हुआ।|Antara muka 3.0.2 digunakan tanpa mengubah sambungan Java, konfigurasi atau profil.' ;;
...
cloud_msg() {
  ...
  case "$1" in
    ...
    config_title) values='Configuration de la Zimlet Cloud 3.0.2 (Nextcloud + serveur bureautique)|Cloud Zimlet 3.0.2 configuration (Nextcloud + office server)|Configuración de la Zimlet Cloud 3.0.2 (Nextcloud + servidor ofimático)|Configurazione della Cloud Zimlet 3.0.2 (Nextcloud + server office)|Konfiguration der Cloud-Zimlet 3.0.2 (Nextcloud + Office-Server)' ;;
    ...
    install_done) values='Installation 3.0.2 terminée. mailboxd est opérationnel et l’extension répond avec la version attendue.|Installation 3.0.2 completed. mailboxd is running and the extension reports the expected version.|Instalación 3.0.2 terminada. mailboxd está operativo y la extensión indica la versión esperada.|Installazione 3.0.2 completata. mailboxd è operativo e l’estensione restituisce la versione prevista.|Installation 3.0.2 abgeschlossen. mailboxd läuft und die Erweiterung meldet die erwartete Version.' ;;
    ...
    clean_modern) values='Déploiement propre de l’interface Modern 3.0.2…|Clean deployment of the Modern 3.0.2 interface…|Despliegue limpio de la interfaz Modern 3.0.2…|Distribuzione pulita dell’interfaccia Modern 3.0.2…|Saubere Bereitstellung der Modern-Oberfläche 3.0.2…' ;;
    ...
    ui_repaired) values='Interface 3.0.2 déployée sans modifier l’extension Java, la configuration ou les profils.|Interface 3.0.2 deployed without changing the Java extension, configuration or profiles.|Interfaz 3.0.2 desplegada sin modificar la extensión Java, la configuración ni los perfiles.|Interfaccia 3.0.2 distribuita senza modificare l’estensione Java, la configurazione o i profili.|Oberfläche 3.0.2 bereitgestellt, ohne Java-Erweiterung, Konfiguration oder Profile zu ändern.' ;;
