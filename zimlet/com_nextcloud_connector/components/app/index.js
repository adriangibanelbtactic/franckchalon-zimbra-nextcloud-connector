...
import { api, archiveUrl, fileUrl, thumbnailUrl, setActiveProfile } from '../../api';
import {
...
export default class App extends Component {
	constructor(props) {
		super(props);
		this.state = initialState(props.workspaceScope);
	}
	...
	componentDidMount() {
		...
		setCloudRouteActive(this.props.workspaceScope, true, globalThis.document);
		this.loadProfile();
	}
	...
	setPageNode = node => {
		...
		if (nextDocument) {
			...
			setCloudRouteActive(this.props.workspaceScope, true, nextDocument);
		}
	...
	downloadFile(file) {
		...
		anchor.href = fileUrl(file.path, 'attachment');
	...
	downloadSelectedArchive = () => {
		...
		const anchor = document.createElement('a');
		anchor.href = archiveUrl(Array.from(parents)[0], items.map(item => item.name));
		anchor.download = 'Cloud-files.zip';
	...
	downloadItem = file => {
		...
		const anchor = document.createElement('a');
		anchor.href = archiveUrl(parentPath(file.path), [file.name]);
